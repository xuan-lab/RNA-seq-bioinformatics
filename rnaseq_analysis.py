...
# Step 5: 差异表达分析 (用 DESeq2 in R)，并保存显著差异基因
def run_deseq2(count_matrix_file, condition_file):
    ...
    write.csv(as.data.frame(sig_res), file="{RESULTS_DIR}/significant_genes.csv")
    """
    
    r(r_code)  # 在 Python 中执行 R 代码
    logging.info("DESeq2 analysis completed. Results and significant genes saved.")
...
if __name__ == "__main__":
    ...
    # 获取修剪后的 FASTQ 文件
    trimmed_fastq_files = [os.path.join(trimmed_output_dir, f) for f in os.listdir(trimmed_output_dir) if f.endswith(".fq")]
